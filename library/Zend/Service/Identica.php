 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Service_Identica extends Zend_Rest_Client
{
     ...
     * OAuth Endpoint
     */
    const OAUTH_BASE_URI = 'http://identi.ca/oauth';
    ...
    protected function _init()
    {
        if (!$this->isAuthorised() && $this->getUsername() !== null) {
            ...
            throw new Zend_Service_Twitter_Exception(
                'Identi.ca session is unauthorised. You need to initialize '
                . 'Zend_Service_Identica with an OAuth Access Token or use '
    ...
    public function statusUpdate($status, $inReplyToStatusId = null)
    {
        ...
        if ($len > self::STATUS_MAX_CHARACTERS) {
            include_once 'Zend/Service/Identica/Exception.php';
            throw new Zend_Service_Identica_Exception(
    ...
    public function userFriends(array $params = array())
    {
        ...
        $response = $this->_get($path, $_params);
file_put_contents(APPLICATION_PATH.'/../tmp/response.log', $response->getBody());
        return new Zend_Rest_Client_Result($response->getBody());
    }
    ...
    protected function _validateScreenName($name)
    {
        if (!preg_match('/^[a-zA-Z0-9_]{0,15}$/', $name)) {
            require_once 'Zend/Service/Identica/Exception.php';
            throw new Zend_Service_Identica_Exception(
                'Screen name, "' . $name
...
//        }
		...
		// Dumb workaround because the /api/ is getting dropped off
		$uri = $this->_uri->getUri();
		if(strpos($uri, '/api/') === false) {
			$this->_uri->setPath('/api'.$path);
		}
